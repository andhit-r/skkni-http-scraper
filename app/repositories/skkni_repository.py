...
def _json_or_raise(r: httpx.Response) -> Any:
    ...
    raise ValueError(
        f"Non-JSON response for {r.request.url}. content-type={ctype} body~200='{snippet}'"
    )
...
def normalize_document(
    ...
    **_: Any,  # serap argumen tak terpakai agar backward-compatible
    ...
    judul = (
        _safe_get(raw, "judul_skkni")
        or _safe_get(raw, "judul")
        or _safe_get(raw, "title")
        or ""
    )
    nomor_skkni = (
        _safe_get(raw, "nomor_skkni")
        or _safe_get(raw, "nomor")
        or _safe_get(raw, "number")
        or ""
    )
    ...
    sektor = (
        _safe_get(raw, "sektor")
        or _safe_get(raw, "taxonomy", "sektor")
        or _safe_get(raw, "sector")
        or ""
    )
    bidang = (
        _safe_get(raw, "bidang")
        or _safe_get(raw, "taxonomy", "bidang")
        or _safe_get(raw, "field")
        or ""
    )
    sub_bidang = (
        _safe_get(raw, "sub_bidang")
        or _safe_get(raw, "taxonomy", "sub_bidang")
        or _safe_get(raw, "subfield")
        or None
    )

    tahun = (
        _safe_get(raw, "tahun")
        or _extract_year(nomor_skkni)
        or _extract_year(nomor_kepmen)
        or ""
    )
    unduh_url = (
        _safe_get(raw, "unduh_url")
        or _safe_get(raw, "download_url")
        or ""
    )
